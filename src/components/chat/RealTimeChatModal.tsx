...
import { Send, Volume2, ArrowLeft, Phone, CheckCheck } from 'lucide-react';
...
export const RealTimeChatModal: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const unsub = dataService.subscribeChatMessages(patientId, (list) => {
      setMessages(list);
      dataService.markChatAsRead(patientId, currentUserRole);
    });
    return () => unsub();
  }, [patientId, currentUserRole]);
  ...
  const otherRoleName = currentUserRole === 'patient' ? 'Caregiver & Medical Team' : 'Patient';
          ...
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400 space-y-2">
              ...
              <p className="text-xs text-slate-400 max-w-xs">
                Type a message below to start real-time DMs with your {otherRoleName}.
              </p>
            </div>
          ) : (
